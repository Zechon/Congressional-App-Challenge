...
public class StaffSelector : MonoBehaviour
{
    ...
    public Dictionary<CampaignRole, List<StaffData>> GetStaffChoices(int countPerRole = 3)
    {
        ...
        while (!valid && safetyCounter < 100)
        {
            ...
            {
                List<StaffData> pool = allStaff
                    .Where(s => s.role == role && !selectedStaffGlobal.Contains(s))
                    .ToList();
            ...
            // Level 5 injection
            if (guaranteeLevel5 || rng.NextDouble() < chanceOfLevel5)
            {
                CampaignRole targetRole = (CampaignRole)System.Enum
                    .GetValues(typeof(CampaignRole))
                    .GetValue(rng.Next(0, System.Enum.GetValues(typeof(CampaignRole)).Length));

                var level5Pool = allStaff
                    .Where(s => s.role == targetRole && s.skill == 5 && !selectedStaffGlobal.Contains(s))
                    .ToList();

                if (level5Pool.Count > 0)
                {
                    int replaceIndex = rng.Next(0, choices[targetRole].Count);
                    var injected = level5Pool[rng.Next(0, level5Pool.Count)];
